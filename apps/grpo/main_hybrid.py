...
# Usage: python -m apps.grpo.main_hybrid --config apps/grpo/qwen3_1_7b_hybrid.yaml
...
async def main(cfg: DictConfig):
    """Main GRPO training loop with hybrid actor (zero weight sync overhead)."""
    ...
    # Initialize torchstore (not used in hybrid mode, but kept for compatibility)
    # In hybrid mode, we don't need torchstore for weight sync, but we keep it
    # for potential future use (e.g., checkpointing)
    hybrid_policy_num_procs = cfg.actors.hybrid_policy["procs"]
    hybrid_policy_host_mesh_name = cfg.actors.hybrid_policy["mesh_name"]
    hybrid_policy_hosts = await provisioner.get_host_mesh(hybrid_policy_host_mesh_name)
    await ts.initialize(
        mesh=hybrid_policy_hosts.spawn_procs(
            per_host={"procs": hybrid_policy_num_procs}
        ),
        strategy=ts.LocalRankStrategy(),
    )
    print("Torchstore initialized (not used for weight sync in hybrid mode)")
    ...
    async def continuous_rollouts():
        ...
        while not shutdown_event.is_set():
            ...
            # Generate using hybrid_policy (automatically switches to inference mode)
            # HybridPolicyActor has multiple procs (FSDP), so use .call() which broadcasts to all ranks
            # .call() returns a ValueMesh - use .item(procs=0) to get result from rank 0
            responses_mesh = await hybrid_policy.generate.call(prompt)
            # All ranks return same result due to synchronized token sampling, so take proc 0
            ...
            for i, response in enumerate(responses):
                ...
                if response.logprobs is None:
                    raise ValueError(
                        ...
                        "Ensure HybridPolicyActor returns logprobs by setting 'logprobs: 1' in sampling_params config."
                    )
    ...
    async def continuous_training():
        ...
        while (
            ...
            if batch is None:
                ...
                # Train using hybrid_policy (automatically switches to training mode)
                await hybrid_policy.train_step.call(batch)
                training_step += 1
                t.step("train_step")

                # NO WEIGHT SYNC NEEDED! Weights are already updated in the same actor
                # The inference_engine uses the same model instance, so changes are immediate
                # This eliminates the 1-3 second bottleneck from push_weights + update_weights
