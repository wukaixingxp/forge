...
class ForgeWorkerWrapper(WorkerWrapper):
    ...
    def apply_gpu_weights(
        ...
        Model-agnostic: delegates all TP slicing and weight merging to
        vLLM's model.load_weights().

        Args:
            gpu_state_dict: Dict mapping param names to either:
                - torch.Tensor: Full tensor (model.load_weights handles TP slicing)
        ...
        model.load_weights(_stream_weights())

        logger.info(
            f"[ForgeWorkerWrapper] Applied {len(gpu_state_dict)} GPU weights (TP rank {tp_rank})"
        )
        return len(gpu_state_dict)
    ...
    def receive_weights_ipc_sliced(
        ...
        """Receive weights via CUDA IPC handles from trainer (per-TP-rank).

        Model-agnostic: each worker selects handles for its TP rank, reconstructs
        tensors, and delegates all TP slicing and weight merging to vLLM's
        model.load_weights().
        ...
        logger.info(f"[IPC-Sliced] Receiving {len(ipc_handles)} weights for TP rank {tp_rank}")
        ...
        def _stream_weights():
            for name, handle in ipc_handles.items():
                try:
                    ...
                    logger.warning(f"[IPC-Sliced] Failed to reconstruct {name}: {e}")

        model.load_weights(_stream_weights())

        logger.info(f"[IPC-Sliced] Loaded {len(ipc_handles)} weights for TP rank {tp_rank}")
        return len(ipc_handles)
    ...
    def receive_shards_ipc(
        ...
        Model-agnostic: delegates all TP slicing, QKV merging, gate/up merging,
        and weight packing to vLLM's model.load_weights(), which handles every
        architecture vLLM supports (Llama, Qwen, Mistral, DeepSeek, etc.).

        Uses a streaming generator with CUDA events for pipelined shard
        reconstruction — GPU copies for param N+1 overlap with load_weights
        processing of param N, eliminating per-param synchronization stalls.
        ...
        param_names = list(shard_handles[first_rank].keys())

        logger.info(
            f"[IPC-Sharded] Worker rank {rank} (TP rank {tp_rank}) receiving "
            f"{len(param_names)} params from {fsdp_size} FSDP ranks for v{policy_version}"
        )

        def _stream_weights():
            """Reconstruct FSDP shards and yield (name, tensor) with pipelined GPU copies.

            Uses CUDA events instead of per-param copy_stream.synchronize():
            - Record an event after issuing non-blocking copies for each param
            - Yield params whose events have already completed (free pipelining)
            - Only synchronize when consuming a param that isn't ready yet
            """
            pending = []  # (param_name, shards, offsets, global_shape, event)
            ...
            for param_name in param_names:
                ...
                if len(shards) != fsdp_size:
                    logger.warning(
                        f"[IPC-Sharded] Incomplete shards for {param_name}: "
                        f"got {len(shards)}/{fsdp_size}"
                    )
                    continue

                global_shape = shard_metadata[first_rank][param_name]["global_shape"]
                event = copy_stream.record_event()
                pending.append((param_name, shards, offsets_list, global_shape, event))

                # Yield any params whose GPU copies are already done (pipelining)
                while pending and pending[0][4].query():
                    name, s, o, gs, _ = pending.pop(0)
                    yield (name, self._combine_shards(s, o, gs))

            # Drain remaining — synchronize only as needed
            for name, s, o, gs, event in pending:
                event.synchronize()
                yield (name, self._combine_shards(s, o, gs))

        # Single call — model.load_weights pulls from the generator lazily.
        # vLLM's AutoWeightsLoader handles all TP slicing, QKV/gate_up merging,
        # MoE routing, and quantization packing for every supported architecture.
        model.load_weights(_stream_weights())

        logger.info(f"[IPC-Sharded] Loaded {len(param_names)} weights for TP rank {tp_rank}")
